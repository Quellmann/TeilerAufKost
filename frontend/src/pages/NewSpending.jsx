...
const NewSpending = () => {
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            setData(data)
            setForm(prevState => ({ ...prevState, to: data.groupMember }))
        }
        (groupId && fetchData())
        return;
    }, [groupId]);
        ...
        <div className='flex-1 overflow-y-auto mt-16 mb-16 2xl:w-[40%] xl:w-[50%] lg:w-[60%] md:w-[70%] sm:w-[80%] w-[90%] mx-auto'>
            <form className='mt-10 flex flex-col' autoComplete='off'>
                ...
                <div className='text-lg pl-3'>
                    <Input onChange={(e) => setForm(prevState => ({ ...prevState, title: e.target.value }))} placeholder='Titel' className="border w-full rounded-lg p-2 text-center" name="spending_title" type="text" />
                </div>
                ...
                <div className='text-lg pl-3 mt-3'>
                    <div className='grid grid-cols-2'>
                        <span>Von</span>
                        <ComboInput data={data} form={form} setForm={setForm} formfield={"from"}></ComboInput>
                    </div>
                    <div className='grid grid-cols-2 mt-24'>
                        <span>An</span>
                        <ComboInput data={data} form={form} setForm={setForm} formfield={"to"} type={"multiple"}></ComboInput>
                    </div>
                </div>
            </form>
